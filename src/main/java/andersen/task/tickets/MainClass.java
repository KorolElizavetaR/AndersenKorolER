...
public class MainClass {	
	...
	public static void main(String[] args) {
		...
		Ticket t1 = service.addTicket(new Ticket("Main hall", 231, new Calendar.Builder()
	            .setDate(2024, 9, 29).setTimeOfDay(19, 00, 0).build(), false, SectorHall.A, 45.6056374)); //Soon will have ConstraintViolation
		...
		service.addTicket(new Ticket("Main hall", 231, new Calendar.Builder()
	            .setDate(2024, 10, 29).setTimeOfDay(19, 00, 0).build(), false, SectorHall.B, 45.6056374));
		
		//service.getAllTickets().stream().forEach(System.out::println);
		service.getTicketBySector(SectorHall.A).stream().forEach(System.out::println);
		try {
			System.out.println(service.getTicketByID(t1.getTicketID()));
		} catch (InstanceNotFoundException e) {
			System.out.printf("Ticket with ID %s is not found", t1.getTicketID());
		}
	}
